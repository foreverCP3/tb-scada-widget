...
export class Device {
    ...
    type: DeviceType = DeviceType.internal;
    ...
    static descriptor: { [key: string]: string } = {
        id: 'Device id, GUID',
        name: 'Device name',
        enabled: 'Enabled',
...
export class Tag {
    ...
    sysType: TagSystemType | null = null;
    /** Description */
    description?: string;
    ...
    static descriptor: { [key: string]: any } = {
        id: 'Tag id, GUID',
...
export class DevicesUtils {
    ...
    static getDeviceFromTagId(devices: Device[], id: string): Device | null {
        for (let i = 0; i < devices.length; i++) {
            if (devices[i].tags[id]) {
                return devices[i];
            }
    ...
    static getTagFromTagId(devices: Device[], id: string): Tag | null {
        for (let i = 0; i < devices.length; i++) {
            if (devices[i].tags[id]) {
    ...
    static getTagFromTagAddress(device: Device, address: string): Tag | undefined {
        return Object.values(device.tags).find((tag: Tag) => tag.address === address);
    }
    ...
    static devicesToCsv(devices: Device[]): string {
        ...
        for (let i = 0; i < devices.length; i++) {
            if (devices[i].tags) {
                const tags = Object.values(devices[i].tags) as Tag[];
    ...
    static csvToDevices(source: string): Device[] | null {
        try {
    ...
    static device2Line(device: Device, dkeys: string[], pkeys: string[]): string {
        ...
        dkeys.forEach(dk => {
            if (dk !== 'property') {
                let text = ((device as any)[dk]) ? (device as any)[dk].toString() : '';
    ...
    static line2Device(line: string, deviceKeyLength: number): Device {
        ...
        device.enabled = items[3].toLowerCase() === 'true';
        device.type = items[4] as DeviceType;
    ...
    //#region Placeholder
    static placeholderToTag(variableId: string, tags: Tag[]): Tag | undefined {
...
export class TagScale {
    mode: TagScaleModeType = TagScaleModeType.undefined;
    rawLow: number = 0;
    rawHigh: number = 100;
    scaledLow: number = 0;
    scaledHigh: number = 100;
